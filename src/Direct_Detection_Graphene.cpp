...
double R_Total_Full_Integral(obscura::DM_Particle& DM, obscura::DM_Distribution& DM_distr, Graphene& graphene, int band, const std::string& method)
{
	...
	libphysica::Vector vel_sun = dynamic_cast<obscura::Standard_Halo_Model*>(&DM_distr)->Get_Observer_Velocity();

	// Order of integrand arguments: q, cos_theta_q, phi_q, cos_theta_kf, phi_kf
	std::function<double(const std::vector<double>&, const double)> integrand = [&DM_distr, &graphene, &DM, band, mDM, vMax, &vel_sun](const std::vector<double>& x, const double wgt) {
		double q			= x[0];
		double cos_theta_q	= x[1];
		double phi_q		= x[2];
		double kf			= x[3];
		...
		Eigen::Vector3d v_unitvector = Spherical_Coordinates_Axis(1.0, acos(cos_theta_v), phi_v, vel_sun);
		...
		double v   = (kf * kf / (2.0 * mDM) - E_l + graphene.work_function + q * q / 2.0 / mDM) / (q * cos_alpha);
		if(v > vMax)
			return 0.0;
		libphysica::Vector vVec({v * v_unitvector[0], v * v_unitvector[1], v * v_unitvector[2]});

		return kf * kf * q * v * v / cos_alpha * DM_distr.PDF_Velocity(vVec) * graphene.DM_Response_corrected(band, qVec, k_FinalVec);
	};
	...
	double result			   = prefactor * libphysica::Integrate_MC(integrand, region, 2000000, method);
